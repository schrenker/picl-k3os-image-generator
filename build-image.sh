...
dl_dep rpi-firmware-nonfree-master.zip https://github.com/RPi-Distro/firmware-nonfree/archive/buster.zip
...
if [ "$IMAGE_TYPE" = "orangepipc2" ]; then
  ...
  BRCMTMP=$(mktemp -d)
  7z e -y deps/rpi-firmware-nonfree-master.zip -o"$BRCMTMP" "firmware-nonfree-buster/brcm/*" > /dev/null
